...
/// `type!(r)` onde `r := Result::Ok 42` → "Result::(Int, E)".
/// O segundo type param E não é resolvido pelo typeck quando só
/// o braço Ok é usado (E seria resolvido via Err ou ascription).
#[test]
fn type_of_generic_two_params() {
    ...
    assert_eq!(
        first, "Result::(Int, E)",
        "type!(r) deve imprimir \"Result::(Int, E)\" — E não resolvido sem Err"
    );
}

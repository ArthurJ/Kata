...
fn bitwise_and_broadcast() {
    ...
    let result = unsafe { kata_rt_bytes_and(a, b, arena) };
    assert_eq!(untag_smi(kata_rt_bytes_len(result)), 2, "resultado tem tamanho do maior");
    assert_eq!(untag_smi(kata_rt_bytes_get(result, 0)), 0xAA, "0xFF AND 0xAA = 0xAA");
    assert_eq!(untag_smi(kata_rt_bytes_get(result, 1)), 0x00, "0xF0 AND 0 (pad) = 0x00");
}
...
fn bitwise_or_broadcast() {
    ...
    assert_eq!(untag_smi(kata_rt_bytes_get(result, 0)), 0xFF, "0xF0 OR 0x0F = 0xFF");
    assert_eq!(untag_smi(kata_rt_bytes_get(result, 1)), 0x0F, "0x0F OR 0 (pad) = 0x0F");
}
...
fn bitwise_xor_broadcast() {
    ...
    let result = unsafe { kata_rt_bytes_xor(a, b, arena) };
    assert_eq!(untag_smi(kata_rt_bytes_len(result)), 2);
    assert_eq!(untag_smi(kata_rt_bytes_get(result, 0)), 0xF0, "0xFF XOR 0x0F = 0xF0");
    assert_eq!(untag_smi(kata_rt_bytes_get(result, 1)), 0x42, "0x42 XOR 0 (pad) = 0x42");
}
...
fn bitwise_and_broadcast_first_shorter() {
    ...
    assert_eq!(untag_smi(kata_rt_bytes_len(result)), 2);
    assert_eq!(untag_smi(kata_rt_bytes_get(result, 0)), 0xAA, "0xFF AND 0xAA = 0xAA");
    assert_eq!(untag_smi(kata_rt_bytes_get(result, 1)), 0x00, "0 (pad) AND 0xBB = 0x00");
}

...
//! `Fork`, `TransmissionOp`, `Select`, ou qualquer nó que produza
...
/// - `TransmissionOp` — operação de canal (impuro)
...
fn check_purity_inner(expr: &TypedExpr) -> Result<(), ComptimeError> {
    match &expr.kind {
        ...
        TypedExprKind::TransmissionOp { .. } => Err(ComptimeError::Impure {
            reason: "contém TransmissionOp".into(),
        }),

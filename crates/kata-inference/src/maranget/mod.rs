...
pub(crate) use guards::collect_guard_leaves;
...
pub(super) fn pattern_ctor(pattern: &TypedPattern) -> Option<Constructor> {
    match pattern {
...
pub(super) struct MatrixRow {
    ...
    pub(super) arm_index: Option<usize>,
}

/// Matriz de pattern-tuples.
pub(super) struct PatternMatrix {
    pub(super) rows: Vec<MatrixRow>,
    /// Tipos das colunas (scrutinee types).
    pub(super) column_tys: Vec<Ty>,
}
...
pub(crate) fn check_exhaustiveness_with_guards(
    ...
    // Precimos percorrer a árvore sem o atalho para identificar as folhas.

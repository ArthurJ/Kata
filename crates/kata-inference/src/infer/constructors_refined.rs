...
fn substitute_hole(expr: &Spanned<Expr>, var_name: &str) -> Spanned<Expr> {
...
fn build_pred_call(pred_name: &str, var_name: &str, base_ty: &Ty) -> Spanned<TypedExpr> {
    ...
    for decl in enum_pred_decls {
        ...
        let body = guards.first().map(|g| g.body.clone()).unwrap_or_else(|| {
            ...
            build_variant_construct(
                &decl.name,
                &decl.variants.last().unwrap().name,
                "x",
                &decl.payload_ty,
                decl.variants.len() - 1,
            )

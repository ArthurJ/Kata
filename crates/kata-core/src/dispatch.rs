...
//! 2. PONTUAR: match_score → Score(exact, iface)
//! 3. ORDENAR: lexicográfico decrescente (exact, iface, !generic)
//! 4. TOPO ÚNICO → Ok
//! 5. EMPATE → AmbiguousDispatch
//!
//! Só `exact` e `iface` são não-zero. Alias→base e refined→base
...
///
/// Ordenação lexicográfica: mais exact vence, depois mais iface.
/// Concreto vence genérico.
...
impl Score {
    ...
    pub fn is_compatible(&self, arg_count: usize) -> bool {
        self.exact + self.iface == arg_count
    }
}
...
/// Conta matches por categoria: exact, iface.
///
...
///
/// Alias→base e refined→base são resolvidos por fallback em

...
/// Wrapper que sobrescreve a severity de um `miette::Diagnostic`.
///
/// Usado quando um pragma `#!warn` rebaixa um diagnóstico de `deny`
/// para `warn`: o erro original é preservado (mensagem, span, código)
/// mas a severity muda para `Warning`, fazendo o miette imprimir como
/// warning em vez de erro.
pub struct SeverityWrapper<T> {
    inner: T,
    severity: miette::Severity,
}

impl<T: miette::Diagnostic> SeverityWrapper<T> {
    pub fn new(inner: T, severity: miette::Severity) -> Self {
        SeverityWrapper { inner, severity }
    }
}

impl<T: std::fmt::Debug> std::fmt::Debug for SeverityWrapper<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("SeverityWrapper")
...
impl<T: std::fmt::Display> std::fmt::Display for SeverityWrapper<T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        self.inner.fmt(f)
    }
}

impl<T: std::error::Error> std::error::Error for SeverityWrapper<T> {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        self.inner.source()
    }
}

impl<T: miette::Diagnostic> miette::Diagnostic for SeverityWrapper<T> {
